...
base_df <- rbind(US_base_df, ak_base_df)
...
write.csv(base_df %>% filter(location == "US") %>% mutate(row = seq(nrow(.))), "tests/testthat/training_data/2022-01-09_addcol.csv", row.names = FALSE)
# scenario
...
write.csv(base_df %>% filter(location == "US") %>% filter(scenario_id == "A-2022-01-09" & location == "US" & target == "1 wk ahead inc death") %>% rbind(.), "tests/testthat/training_data/2022-01-09_doublepoint.csv", row.names = FALSE)
