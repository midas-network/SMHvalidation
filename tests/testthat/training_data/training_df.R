...
US_base_df <- expand.grid(location =  c("US"),
  ...
  mutate(output_type = ifelse(is.na(output_type_id), "median", "quantile"),
         value = case_when(
           grepl("inc case", target) & output_type_id == 0.5 ~ 5000000 * horizon,
           grepl("cum case", target) & output_type_id == 0.5 ~ 65000000 * horizon,
           grepl("inc death", target) & output_type_id == 0.5 ~ 12000 * horizon,
           grepl("cum death", target) & output_type_id == 0.5 ~ 850000 * horizon,
           grepl("inc hosp", target) & output_type_id == 0.5 ~ 150000 * horizon,
           grepl("cum hosp", target) & output_type_id == 0.5 ~ 1 * horizon),
...
US_base_df <- lapply(lst_df, function(x) {
  df <- mutate(x, value = ifelse(output_type == "median",
                                 x[which(output_type_id == 0.5), "value"], value))
  sort_type_id <- sort(na.omit(unique(df$output_type_id)))
  for (i in 1:(length(sort_type_id))) {
...
ak_base_df <- expand.grid(location =  c("02"),
  ...
  mutate(output_type = ifelse(is.na(output_type_id), "median", "quantile"),
         value = case_when(
           grepl("inc case", target) & output_type_id == 0.5 ~ 2700 * horizon,
           grepl("cum case", target) & output_type_id == 0.5 ~ 141000 * horizon,
           grepl("inc death", target) & output_type_id == 0.5 ~ 30 * horizon,
           grepl("cum death", target) & output_type_id == 0.5 ~ 855 * horizon,
           grepl("inc hosp", target) & output_type_id == 0.5 ~ 100 * horizon,
           grepl("cum hosp", target) & output_type_id == 0.5 ~ 1 * horizon),
         value = ifelse(horizon > 1 & horizon < 7, value / 2, value),
         value = ifelse(horizon > 7, value * 0.1, value),
...
al_base_df <- expand.grid(location =  c("01"),
  ...
  mutate(output_type = ifelse(is.na(output_type_id), "median", "quantile"),
         value = case_when(
           grepl("inc case", target) & output_type_id == 0.5 ~ 9000 * horizon,
           grepl("cum case", target) & output_type_id == 0.5 ~ 700000 * horizon,
           grepl("inc death", target) & output_type_id == 0.5 ~ 4600 * horizon,
           grepl("cum death", target) & output_type_id == 0.5 ~ 62100 * horizon,
           grepl("inc hosp", target) & output_type_id == 0.5 ~ 8000 * horizon,
           grepl("cum hosp", target) & output_type_id == 0.5 ~ 1 * horizon),
         value = ifelse(horizon > 1 & horizon < 7, value / 2, value),
         value = ifelse(horizon > 7, value * 0.1, value),
...
# col test
write.csv(base_df %>% filter(location == "US", is.na(output_type_id)) %>% rename(fips = location), "tests/testthat/training_data/2022-01-09_colname.csv", row.names = FALSE)
...
# Origin date
write.csv(base_df %>% filter(location == "US") %>% mutate(origin_date = rep(c("2022-01-09", "2022-01-11"), each = nrow(.)/2)), "tests/testthat/training_data/2022-01-09_multimpd.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% mutate(origin_date = as.Date("2922-01-09")), "tests/testthat/training_data/2022-01-09_mpd_error.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% mutate(origin_date = "01/09/2022"), "tests/testthat/training_data/2022-01-09_mpd_format.csv", row.names = FALSE)
# output_type_ids
write.csv(base_df %>% filter(location == "US") %>% mutate(output_type_id = gsub(0.01, 0.02, output_type_id)), "tests/testthat/training_data/2022-01-09_badquant.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% filter(!grepl("0\\.5$", output_type_id)), "tests/testthat/training_data/2022-01-09_missquant.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% filter(!grepl("^0$|^1$", output_type_id)), "tests/testthat/training_data/2022-01-09_missoptquant.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% mutate(value = ifelse(scenario_id == "A-2022-01-09" & output_type_id == 1 & location == "US" & target == "inc death", 0, value)), "tests/testthat/training_data/2022-01-09_valquant.csv", row.names = FALSE) # NA for median value and value decreasing for last quantiles
# value
write.csv(base_df %>% filter(location == "US") %>% mutate(output_type = ifelse(is.na(output_type_id) & location == "US", "pnt", output_type)), "tests/testthat/training_data/2022-01-09_badpointtype.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% mutate(output_type_id = ifelse(grepl("median", output_type) & location == "US", 0.5, output_type_id)), "tests/testthat/training_data/2022-01-09_badpointquant.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% mutate(value = ifelse(scenario_id == "A-2022-01-09" & grepl("^0$", output_type_id) & location == "US" & target == "inc death", -10, value)), "tests/testthat/training_data/2022-01-09_negvalue.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% rbind(filter(., scenario_id == "A-2022-01-09" & location == "US" & target == "inc death")), "tests/testthat/training_data/2022-01-09_doublepoint.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% rbind(filter(., scenario_id == "A-2022-01-09" & location == "US" & target == "inc death", output_type_id == 0)), "tests/testthat/training_data/2022-01-09_doublequantzero.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% mutate(value = ifelse(scenario_id == "A-2022-01-09" & grepl("^1$", output_type_id) & location == "US" & target == "inc death", 1e9, value)), "tests/testthat/training_data/2022-01-09_highvalue.csv", row.names = FALSE)
...
write.csv(base_df %>% filter(location == "US") %>% mutate(value = ifelse(scenario_id == "A-2022-01-09" & location == "US" & target == "cum case", 10, value)), "tests/testthat/training_data/2022-01-09_lowcumcase.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% mutate(value = ifelse(scenario_id == "A-2022-01-09" & location == "US" & target == "cum death", 1, value)), "tests/testthat/training_data/2022-01-09_lowcumdeath.csv", row.names = FALSE)
# target
write.csv(base_df %>% filter(location == "US") %>% mutate(target = gsub("inc case", "inccase", target)), "tests/testthat/training_data/2022-01-09_badnametarget.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% filter(!grepl(12, horizon)), "tests/testthat/training_data/2022-01-09_misswk.csv", row.names = FALSE)
...
write.csv(base_df %>% filter(location == "02" & !(output_type_id %in% c(0, 1))) %>% mutate(location = gsub("02", "2", location)), "tests/testthat/training_data/2021-11-14_no0location.csv", row.names = FALSE)
...
# Truth data
#lst_gs <- pull_gs_data()
#saveRDS(lst_gs, "tests/testthat/training_data/2022-08-22_lst_gs.rds")
...
US_base_df <- expand.grid(location =  c("US"),
  ...
  mutate(output_type = ifelse(is.na(output_type_id), "median", "quantile"),
         value = case_when(
           grepl("peak time", target) & output_type_id == 0.5 ~ 0.1 * horizon - 3.2,
           grepl("peak size", target) & output_type_id == 0.5 ~ 65000000,
           grepl("inc death", target) & output_type_id == 0.5 ~ 12000 * horizon,
           grepl("cum death", target) & output_type_id == 0.5 ~ 1 * horizon,
           grepl("inc hosp", target) & output_type_id == 0.5 ~ 150000 * horizon,
           grepl("cum hosp", target) & output_type_id == 0.5 ~ 1000000 * horizon),
...
US_base_df <- filter(US_base_df, !(grepl("peak time", target) &
  ...
  mutate(output_type_id = ifelse(grepl("cdf", output_type), NA, output_type_id)) %>%
...
al_base_df <- filter(US_base_df, grepl("hosp", target)) %>%
  ...
  mutate(value = ifelse(grepl("time", target) & horizon > 1, value + 0.5, value)) %>%
  mutate(value = ifelse(grepl("time", target) & horizon > 20, value + 0.05, value)) %>%
  mutate(value = ifelse(grepl("time", target) & horizon > 40, value + 1, value)) %>%
  mutate(value = ifelse(grepl("time", target) & value >= 1, 1, value)) %>%
  mutate(value = ifelse(grepl("time", target) & value <= 0, 0, value))
...
US_samp_df <- expand.grid(location =  c("US"),
  ...
  mutate(value = case_when(
           grepl("inc death", target) & output_type_id == 1 ~ 12000 * horizon,
           grepl("inc hosp", target) & output_type_id == 1 ~ 150000 * horizon),
         value = ifelse(scenario_id == "B-2022-08-14", value * 1.05, value),
         value = ifelse(scenario_id == "C-2022-08-14", value * 1.1, value),
         value = ifelse(scenario_id == "D-2022-08-14", value * 1.15, value))
...
write.csv(tot[!(grepl("median", tot$output_type) & !grepl("time", tot$target)),], "tests/testthat/training_data/2022-08-14_flu_nopoint_noerror.csv", row.names = FALSE)
...
write.csv(dplyr::filter(tot_s, output_type_id < 90, !grepl("inc death", target)) %>% dplyr::mutate(output_type_id = ifelse(output_type_id > 85, 104.5, output_type_id)), "tests/testthat/training_data/2022-08-14_flu_badsample.csv", row.names = FALSE)
