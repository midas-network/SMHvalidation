...
write.csv(base_df %>% filter(location == "US") %>% mutate(value = ifelse(scenario_id == "A-2022-01-09" & quantile == 1 & location == "US" & target == "1 wk ahead inc death", 0, value)), "tests/testthat/training_data/2022-01-09_valquant.csv", row.names = FALSE) # NA for point value and value decreasing for last quantiles
# value
...
write.csv(base_df %>% filter(location == "US") %>% mutate(value = ifelse(scenario_id == "A-2022-01-09" & grepl("^0$", quantile) & location == "US" & target == "1 wk ahead inc death", -10, value)), "tests/testthat/training_data/2022-01-09_negvalue.csv", row.names = FALSE)
...
write.csv(base_df %>% filter(location == "US") %>% mutate(value = ifelse(scenario_id == "A-2022-01-09" & grepl("^1$", quantile) & location == "US" & target == "1 wk ahead inc death", 1e9, value)), "tests/testthat/training_data/2022-01-09_highvalue.csv", row.names = FALSE)
...
target_num <- unique(gsub("\\d+ wk ahead peak size", "peak size", target_num))
...
US_base_df <- expand.grid(location =  c("US"),
  ...
  mutate(target_end_date = as.Date(model_projection_date) +
         ...
         value = case_when(
           grepl(" wk .+ peak time", target) & quantile == 0.5 ~ 0.1 * (as.numeric(gsub("[^[:digit:]]", "", target))) - 3.2,
           grepl("peak size", target) & quantile == 0.5 ~ 65000000,
           grepl(" wk .+ inc death", target) & quantile == 0.5 ~ 12000 * (as.numeric(gsub("[^[:digit:]]", "", target))),
           grepl(" wk .+ cum death", target) & quantile == 0.5 ~ 1 * (as.numeric(gsub("[^[:digit:]]", "", target))),
           grepl(" wk .+ inc hosp", target) & quantile == 0.5 ~ 150000 * (as.numeric(gsub("[^[:digit:]]", "", target))),
           grepl(" wk .+ cum hosp", target) & quantile == 0.5 ~ 1000000 * (as.numeric(gsub("[^[:digit:]]", "", target)))),
         value = ifelse(!grepl("size", target) & as.numeric(gsub("[^[:digit:]]", "", target)) > 1 & as.numeric(gsub("[^[:digit:]]", "", target)) < 7, value / 2, value),
         value = ifelse(!grepl("size", target) & as.numeric(gsub("[^[:digit:]]", "", target)) > 7, value * 0.1, value),
...
US_base_df <- filter(US_base_df, !(grepl("peak time", target) & grepl("quantile", type))) %>%
  mutate(value = ifelse(value < 0, 0, value)) %>%
...
al_base_df <- filter(US_base_df, grepl("hosp", target)) %>%
  mutate(value = value * 0.00001,
         location = "02")  %>%
  mutate(value = ifelse(grepl("time", target) & target_end_date > "2022-12-31", value + 0.5, value)) %>%
  mutate(value = ifelse(grepl("time", target) & target_end_date > "2023-03-04", value + 0.5, value)) %>%
  mutate(value = ifelse(grepl("time", target) & value >= 1, 1, value)) %>%
  mutate(value = ifelse(grepl("time", target) & value <= 0, 0, value))
