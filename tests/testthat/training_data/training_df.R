...
US_base_df <- expand.grid(location =  c("US"),
                          ...
                          target = target_name,
                          horizon = seq(1:12)) %>%
  mutate(type = ifelse(is.na(type_id), "median", "quantile"),
         value = case_when(
           grepl("inc case", target) & type_id == 0.5 ~ 5000000 * horizon,
           grepl("cum case", target) & type_id == 0.5 ~ 65000000 * horizon,
           grepl("inc death", target) & type_id == 0.5 ~ 12000 * horizon,
           grepl("cum death", target) & type_id == 0.5 ~ 850000 * horizon,
           grepl("inc hosp", target) & type_id == 0.5 ~ 150000 * horizon,
           grepl("cum hosp", target) & type_id == 0.5 ~ 1 * horizon),
...
US_base_df <- lapply(lst_df, function(x) {
  for (i in 2:12) {
    if (grepl("cum ", unique(x$target))) {
      x[which(x$type_id == 0.5 & x$horizon == i), "value"] <-
        x[which(x$type_id == 0.5 & x$horizon == (i - 1)), "value"] +
        (x[which(x$type_id == 0.5 & x$horizon == i), "value"]/10)
    }
    x
  }
  x
...
US_base_df <- lapply(lst_df, function(x) {
  df <- mutate(x, value = ifelse(type == "median",
                                 x[which(type_id == 0.5), "value"], value))
  sort_type_id <- sort(na.omit(unique(df$type_id)))
  for (i in 1:(length(sort_type_id))) {
    if (is.na(df[which(df$type_id == sort_type_id[i]), "value"])) {
      if (sort_type_id[i] < 0.5 & sort_type_id[i] > 0) {
        df[which(df$type_id == sort_type_id[i]), "value"] <-
          df[which(df$type_id == 0.5), "value"] - 1/sort_type_id[i]
      } else if (sort_type_id[i] < 0.5 & sort_type_id[i] == 0) {
        df[which(df$type_id == sort_type_id[i]), "value"] <-
          df[which(df$type_id == 0.5), "value"] - 1/0.005
      } else {
        df[which(df$type_id == sort_type_id[i]), "value"] <-
          sort_type_id[i] * df[which(df$type_id == 0.5), "value"] +
          df[which(df$type_id == 0.5), "value"]
      }
    }
    df
  }
...
ak_base_df <- expand.grid(location =  c("02"),
  ...
  mutate(type = ifelse(is.na(type_id), "median", "quantile"),
         value = case_when(
           grepl("inc case", target) & type_id == 0.5 ~ 2700 * horizon,
           grepl("cum case", target) & type_id == 0.5 ~ 141000 * horizon,
           grepl("inc death", target) & type_id == 0.5 ~ 30 * horizon,
           grepl("cum death", target) & type_id == 0.5 ~ 855 * horizon,
           grepl("inc hosp", target) & type_id == 0.5 ~ 100 * horizon,
           grepl("cum hosp", target) & type_id == 0.5 ~ 1 * horizon),
         value = ifelse(horizon > 1 & horizon < 7, value / 2, value),
         value = ifelse(horizon > 7, value * 0.1, value),
...
al_base_df <- expand.grid(location =  c("01"),
  ...
  mutate(type = ifelse(is.na(type_id), "median", "quantile"),
         value = case_when(
           grepl("inc case", target) & type_id == 0.5 ~ 9000 * horizon,
           grepl("cum case", target) & type_id == 0.5 ~ 700000 * horizon,
           grepl("inc death", target) & type_id == 0.5 ~ 4600 * horizon,
           grepl("cum death", target) & type_id == 0.5 ~ 62100 * horizon,
           grepl("inc hosp", target) & type_id == 0.5 ~ 8000 * horizon,
           grepl("cum hosp", target) & type_id == 0.5 ~ 1 * horizon),
         value = ifelse(horizon > 1 & horizon < 7, value / 2, value),
         value = ifelse(horizon > 7, value * 0.1, value),
...
# col test
write.csv(base_df %>% filter(location == "US", is.na(type_id)) %>% rename(fips = location), "tests/testthat/training_data/2022-01-09_colname.csv", row.names = FALSE)
...
# type_ids
write.csv(base_df %>% filter(location == "US") %>% mutate(type_id = gsub(0.01, 0.02, type_id)), "tests/testthat/training_data/2022-01-09_badquant.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% filter(!grepl("0\\.5$", type_id)), "tests/testthat/training_data/2022-01-09_missquant.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% filter(!grepl("^0$|^1$", type_id)), "tests/testthat/training_data/2022-01-09_missoptquant.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% mutate(value = ifelse(scenario_id == "A-2022-01-09" & type_id == 1 & location == "US" & target == "inc death", 0, value)), "tests/testthat/training_data/2022-01-09_valquant.csv", row.names = FALSE) # NA for median value and value decreasing for last quantiles
# value
write.csv(base_df %>% filter(location == "US") %>% mutate(type = ifelse(is.na(type_id) & location == "US", "pnt", type)), "tests/testthat/training_data/2022-01-09_badpointtype.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% mutate(type_id = ifelse(grepl("median", type) & location == "US", 0.5, type_id)), "tests/testthat/training_data/2022-01-09_badpointquant.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% mutate(value = ifelse(scenario_id == "A-2022-01-09" & grepl("^0$", type_id) & location == "US" & target == "inc death", -10, value)), "tests/testthat/training_data/2022-01-09_negvalue.csv", row.names = FALSE)
...
write.csv(base_df %>% filter(location == "US") %>% rbind(filter(., scenario_id == "A-2022-01-09" & location == "US" & target == "inc death", type_id == 0)), "tests/testthat/training_data/2022-01-09_doublequantzero.csv", row.names = FALSE)
write.csv(base_df %>% filter(location == "US") %>% mutate(value = ifelse(scenario_id == "A-2022-01-09" & grepl("^1$", type_id) & location == "US" & target == "inc death", 1e9, value)), "tests/testthat/training_data/2022-01-09_highvalue.csv", row.names = FALSE)
...
US_base_df <- expand.grid(location =  c("US"),
  ...
  mutate(type = ifelse(is.na(type_id), "median", "quantile"),
         value = case_when(
           grepl("peak time", target) & type_id == 0.5 ~ 0.1 * horizon - 3.2,
           grepl("peak size", target) & type_id == 0.5 ~ 65000000,
           grepl("inc death", target) & type_id == 0.5 ~ 12000 * horizon,
           grepl("cum death", target) & type_id == 0.5 ~ 1 * horizon,
           grepl("inc hosp", target) & type_id == 0.5 ~ 150000 * horizon,
           grepl("cum hosp", target) & type_id == 0.5 ~ 1000000 * horizon),
...
US_base_df <- filter(US_base_df, !(grepl("peak time", target) &
  ...
  mutate(value = ifelse(grepl("time", target) & value <= 0, 0, value),
         age_group = "0-130") %>%
  mutate(type = ifelse(grepl("time", target), "cdf", type)) %>%
  mutate(type_id = ifelse(grepl("cdf", type), NA, type_id)) %>%
...
US_samp_df <- expand.grid(location =  c("US"),
  ...
  mutate(value = case_when(
           grepl("inc death", target) & type_id == 1 ~ 12000 * horizon,
           grepl("inc hosp", target) & type_id == 1 ~ 150000 * horizon),
         value = ifelse(scenario_id == "B-2022-08-14", value * 1.05, value),
         value = ifelse(scenario_id == "C-2022-08-14", value * 1.1, value),
         value = ifelse(scenario_id == "D-2022-08-14", value * 1.15, value))
