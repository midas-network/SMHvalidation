    ...
    if (dim(df_test)[1] > 0) {
      ...
      if (isFALSE(all(age_vect %in% all_agegroup))) {
        age_all <- paste0(
          ...
          "value: '", paste(age_vect[!(age_vect %in% all_agegroup)],
                            collapse = "', '"),  "'. 'age_group' can only be: ",
          paste(all_agegroup, collapse = ', '), ", for the target: ",
      ...
      if (isFALSE(all(req_agegroup %in% age_vect))) {
        age_req <- paste0(
          "\U000274c Error 802: The `age_group` column is missing at least 1 ",
          " required value: '",
          paste(req_agegroup[!(req_agegroup %in% age_vect)], collapse = "', '"),
          "'. 'age_group' should contain: ",
          paste(req_agegroup, collapse = ', '), ", for the target: ",
          paste(unique(unlist(x$task_ids$target)),collapse = ", "), ".")
      } else {
        age_req <- NA
      }
