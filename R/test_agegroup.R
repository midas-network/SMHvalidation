...
test_agegroup <- function(df, js_def) {
  ...
  lst_df <-  split(df, as.list(df[,sel_group]))
  ...
  overall_test <- lapply(lst_df, function(x) {
    ...
    if (length(add_age) > 1) {
      if (any(!(seq(1, 130) %in% sort(unique(unlist(
    ...
    all_test <- unique(sub_group, overall_test)
    return(all_test)
  })
  ...
  #- targets woth specific age_group does not contains additional age_group
  age_target <- names(purrr::keep(purrr::map(
    c(js_def$required_targets, js_def$optional_targets), "age_group"),
    function(x) x != "all"))
  targ_age_test <- lapply(age_target, function(x) {
    test <- c(js_def$required_targets, js_def$optional_targets)[[x]]$age_group
    df_test <- dplyr::filter(df, !grepl(paste(test, collapse = "|"), age_group),
                             grepl(x, target))
    if (dim(df_test)[1] > 0) {
      age_test <- paste0(
        "\U000274c Error 806: The target ", x, " should only contain the ",
