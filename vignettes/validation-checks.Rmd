...
*Remarks:* After round 16 of COVID-19, the submission file format and js_def
file format has been updated to adapt to future rounds. The documentation is 
about the last version of the data. For previous version, please consult past
...
| origin_date | scenario_id | target | horizon | location | type | type_id | value |
|:------:|:------:|:------:|:------:|:------:|:------:|:------:|:------:|
...
-   The ID of the scenarios are corresponding to the expected
    ID of the expected round without any typo (information
...
-   The `origin_date` column contains:
    ...
    -   the date in the submission file matching the projection starting
        date or round id (information should be provided via the `js_def`
        parameter).
...
-   Each type/type_id/target/scenario/location/(age_group, etc.) group
    combination has one unique value projected. For example: only 1
    value for quantile `0.5`, location `US`, target
    `inc hosp`, horizon `1` and, scenario `A`)

-   The projection contains the expected value (for example, integer value
    `>= 0` or double in between 0 and 1, information should be provided 
    via the `js_def` parameter)
...
-   The submission file contains projection for an expected number of
    week (or horizon) (information should be provided via the `js_def`
     parameter). **If the file contains more projected weeks than expected, 
    the submission will still be accepted, but will return a warning message
...
-   The column `type_id` for the type ‘sample’ rows should only contain 
    expected integer values (information should be provided via the `js_def` 
    parameter).
...
| 201 - [Deprecated for new files format]       | Error            | Run in `test_scenario()` function                                                                                 | The `scenario_name` values are correct.                                                                                                                                                                                                                                              |
| 202                                           | Error            | Run in `test_scenario()` function                                                                                 | The `scenario_id` values are correct.                                                                                                                                                                                                                                                |
| 203 - [Deprecated for new files format]       | Error            | Run in `test_scenario()` function                                                                                 | The name and ID of the scenario are correctly associated.                                                                                                                                                                                                                            |
| 301                                           | Error            | Run in `test_origindate()` function                                                                                 | The `origin_date` value is in a date format "YYYY-MM-DD".                                                                                                                                                                                                                  |
...
| 304 - [Deprecated for new files format]       | Error            | Run in `test_origindate()` function                                                                                 | The `origin_date` value is corresponding to the projection starting date.                                                                                                                                                                                                  |
...
| 501 - [Deprecated for new files format]       | Error            | Run in `test_val()` function                                                                                      | The projection contains type `"point"` values for the target(s) requiring "point" values.                                                                                                                                                                                            |
| 502 - [Deprecated for new files format]       | Error            | Run in `test_val()` function                                                                                      | The `"point"` type values have for value `NA` in the column `quantile`.                                                                                                                                                                                                              |
| 503 - [Deprecated for new files format]       | Error            | Run in `test_val()` function                                                                                      | The projection contains X point value, X = number of targets \* number of scenarios \* number of locations (only when all required targets required point projections for all scenarios, locations)                                                                                  |
| 5041                                          | Error            | Run in `test_val()` function                                                                                      | All `value` have the expected value (for example, numeric \>= 0 or in between 0 and 1).                                                                                                                                                                                             |
| 5042                                          | Error            | Run in `test_val()` function                                                                                      |`NA` values are not accepted.                                                                                                                                                                                                                               |
| 505                                           | Warning          | Run in `test_val()` function                                                                                      | For each group of location/target/scenario (and age_group, etc. if necessary) , the projected `value` are not identical for the complete time series.                                                                                                                                      |
| 506 - [Deprecated for new files format]       | Error or Warning | Run in `test_val()` function                                                                                      | For each group of location/target/scenario (and age_group if necessary) and for each target requiring "point" values , an unique `"point"` is associated with each group. The test will return an error message for required target(s) and a warning message for optional target(s). |
...
| 510                                           | Error            | Run in `test_val()` function                                                                                      | For each group of location/target/type/type_id/scenario (and age_group,etc. if necessary), no more than one value is associated                                                                                                                                                               |
...
| 512                                           | Error            | Run in `test_val()` function                                                                                      | The submission file contains output in the expected `type` format (for example: quantile, sample, etc.).                                                                                                      |
...
| 603 - [Deprecated for new files format]       | Error            | Run in `test_target()` function                                                                                   | Each `target_end_date` corresponds to the end of an epiweek (Saturday).                                                                                                                                                                                                              |
...
| 608 - [Deprecated for new files format]       | Error            | Run in `test_target()` function                                                                                   | The first week `target_end_date` corresponds to the expected value.                                                                                                                                                                                                                  |
| 609 - [Deprecated for new files format]       | Error            | Run in `test_target()` function                                                                                   | Each `target_end_date` corresponds to the expected value.                                                                                                                                                                                                                            |
...
| 612                                           | Error            | Run in `test_target()` function                                                                                   | The projection contains NA for all "target_end_date" or"horizon" for the target requiring no time series information.                                                                                                                                                                            |
...
| 703                                           | Error            | Run in `test_location()` function                                                                                 | For the target(s) requiring only specific location(s), no location is missing or no additional location is provided in the submission file                                                                                                                                                                        |
...
| 901                                           | Warning          | Run in `test_sample()` function                                                                                   | The column `type_id` for the type 'sample' rows contains expected value                                                                                                                                                                                                             |
| 902                                           | Error            | Run in `test_sample()` function                                                                                   | The submission should contain a unique sample identifier for each scenario/target/location (age_group, etc.) group.                                                                                                                                                                        |
