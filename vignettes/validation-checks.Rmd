 ...
 * The target are corresponding to the target name as expressed in the SMH 
 ...
 optional target has been added: `"inc inf"`. Also, for round 14 only, an other
 additional optional target is possible: `"prop X"`
 ...
 * Only for Round 14 and optional target `prop X`, the submission file should 
 contains only point value between 0 and 1 for this target. The target can be 
 missing for the Scenario A and C, as the Variant X is not present in these 2 
 scenarios. The `prop X` target can also be provided for a subset of location, 
 but a warning message will be return. 
...
|522|Error|Run in `test_val()` function|For each quantile/target name/scenario/location combination (for the cumulative target only), the projected values are not decreasing with time|
