...
For the option (2), the optional parameter `merge_sample_col` cam be used to
...
-   Each task id columns values are corresponding to the expected ID of the
...
-   If the submission file contains quantiles. For each task_id group, the 
...
-   `hub_path`: path to the repository containing the submissions files and
    the `tasks.json` file, in the hubverse format.
-   `folder_path`: path to the folder containing the partitioned submission
    files. The folder is expected to be situated in the `model-output` folder 
    and the path here should be relative to the `hub_path`, model output folder.
