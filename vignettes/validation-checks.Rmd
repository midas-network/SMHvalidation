...
The documentation is about the last version of the data.

For previous version, please consult past version of the package
(version 0.0.0.9).
...
The `validate_submission()` requires two parameters:
...
documentation at `?validate_submision()`
...
-  The submission file(s) should be in one of the accepted format:
...
-   [optional] The number of decimal point accepted in the column `"value"` is
...
-   For some targets, the submission is expected to contains projection for an
    expected number of week (or horizon). **If the file contains more projected
    weeks than expected, the submission will still be accepted, but will return
    a warning message and the additional weeks will not be included in the
...
-   For some targets, for example `peak size hosp`, no `"horizon"` information
    is expected. In this case, the column `"horizon"` should be contain `NA`

### Column "location" (`test_location()`)

-   The location are corresponding to the location name as expressed in the
    associated metadata JSON file. If FIPS code are expected and if the FIPS
...
-   The submission should contain the expected `age_group` information,
    as specified in the associated `tasks.json` file
...
-   The submission should contain the expected `race_ethinicity` information,
    as specified in the associated `tasks.json` file
...
-   The submission should contain the expected number of trajectories for each
    task_id group, as indicated in the associated `tasks.json` file. And each
    group should contain the same number of trajectories.
